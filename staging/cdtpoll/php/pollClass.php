...
class Poll { 
    ...
    public function getPoll($lang){
		...
		//get question
		if($lang == "fr"){
			$stmt->prepare( "SELECT id,question from poll_question_fr where id=? and deleted =0");
		}else{
		...
		//get options
		if($lang == "fr"){
			$stmt->prepare( "SELECT id,poll_question_id,optionText from poll_options_fr where poll_question_id=? and deleted=0");
		}else{
	...
	public function getPollResults($lang){
		if(!$this->validateID($this->ID)){ return "No poll ID found"; }
		$return = array("id" => $this->getID(), "question"=>$this->getQuestion($lang), "options"=>array());
		$return['total'] = 0;
		$stmt = $this->con->stmt_init();
		if($lang == "fr"){
			$stmt->prepare( "SELECT po.id,po.poll_question_id,IF(ISNULL(`resultShortCopy`),`optionText`,IF(STRCMP(`resultShortCopy`,''),`resultShortCopy`,`optionText`))`optionText`,(count(*)-IF(ISNULL(time),1,0)) 'count' FROM `poll_options_fr` po left join poll_answers pa on po.id = pa.poll_options_id where po.poll_question_id =? and po.deleted=0 group by (`optionText`) ORDER BY po.id");
		}else{
